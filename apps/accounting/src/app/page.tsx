		...
		<div className="relative flex min-h-screen flex-col bg-slate-50 selection:bg-primary/20 overflow-hidden">
			...
			<header className="relative z-50 w-full border-b border-white/40 bg-white/60 backdrop-blur-xl">
				<div className="container mx-auto flex h-20 items-center justify-between px-6 md:px-12">
					...
					<Button
						...
						variant="default"
						size="sm"
						className="h-10 rounded-full px-6 font-semibold shadow-sm transition-transform hover:scale-105 active:scale-95"
					>
				...
				<section className="container mx-auto px-6 py-12 md:px-12 text-center">
					<div className="mx-auto flex max-w-4xl flex-col items-center">
						...
						<p className="mx-auto mt-8 max-w-2xl text-lg font-medium leading-relaxed text-slate-600 sm:text-xl">
							The centralized accounting platform for {SCHOOL_INFO.name}. Manage
							ledgers, student billing, tuition plans, and employee payroll in
							one secure portal.
						</p>
						...
						<div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
							<Button
								asChild
								size="lg"
								className="group h-14 rounded-full bg-primary px-8 text-base font-bold shadow-xl shadow-primary/30 transition-all hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/40 active:translate-y-0"
							>
								<Link href="/login">
									Access Portal
			...
			<footer className="relative z-10 py-8">
				<div className="container mx-auto px-6 text-center text-xs font-semibold uppercase tracking-wider text-slate-400 md:px-12">
					<p>
						&copy; {new Date().getFullYear()} {SCHOOL_INFO.name}. All rights
						reserved.
					</p>
				</div>
			</footer>
