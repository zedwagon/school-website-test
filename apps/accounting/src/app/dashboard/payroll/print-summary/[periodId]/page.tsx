...
export default async function PrintPayrollSummaryPage({
	...
	const totalBase = payrolls.reduce(
		(acc, r) => acc + parseFloat(r.baseSalary),
		0,
	);
	const totalEarnings = payrolls.reduce(
		(acc, r) => acc + parseFloat(r.totalEarnings),
		0,
	);
	const totalDeductions = payrolls.reduce(
		(acc, r) => acc + parseFloat(r.totalDeductions),
		0,
	);
	const totalNet = payrolls.reduce((acc, r) => acc + parseFloat(r.netPay), 0);

	const formatMoney = (val: number | string) => {
		return parseFloat(val as string).toLocaleString("en-US", {
				...
				<div className="flex items-center gap-4 mb-6 mt-4">
					...
					<div>
						...
						<p className="text-sm text-gray-600">
							Payroll Summary for {period.name}
						</p>
					</div>
				</div>
				...
				<table className="w-full text-sm border-collapse border border-gray-300">
					...
					<tbody>
						{payrolls.map((record: any) => {
							...
							const baseOrDaily = isPartTime
								? parseFloat(record.dailySalary)
								: parseFloat(record.baseSalary);
							const allowances = isPartTime
								? 0
								: parseFloat(record.totalEarnings) -
									parseFloat(record.baseSalary);
							...
							return (
								<tr key={record.id}>
									...
									<td className="border border-gray-300 px-2 py-1 text-right">
										{isPartTime
											? `${formatMoney(baseOrDaily)}/day`
											: formatMoney(baseOrDaily)}
									</td>
									<td className="border border-gray-300 px-2 py-1 text-right">
										{isPartTime ? "-" : formatMoney(allowances)}
									</td>
									<td className="border border-gray-300 px-2 py-1 text-right">
										{formatMoney(record.totalEarnings)}
									</td>
									<td className="border border-gray-300 px-2 py-1 text-right">
										{formatMoney(record.totalDeductions)}
									</td>
						...
						<tr className="bg-gray-50 print:bg-gray-100 font-bold">
							...
							<td className="border border-gray-300 px-2 py-1 text-right">
								{formatMoney(totalEarnings - totalBase)}
							</td>
							<td className="border border-gray-300 px-2 py-1 text-right">
