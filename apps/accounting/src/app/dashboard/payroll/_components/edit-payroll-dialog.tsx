...
import { AlertCircle, Edit2, Receipt, Wallet } from "lucide-react";
			...
			<DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto sm:rounded-2xl">
				<DialogHeader className="mb-4">
					<DialogTitle className="text-2xl text-slate-800">
						Edit Payroll for {employee?.firstName} {employee?.lastName}
					</DialogTitle>
					<DialogDescription className="text-slate-500">
						Modify earnings or add one-time deductions (like loans) for this
						specific period.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="mb-6 p-4 bg-amber-50/80 border border-amber-200/80 rounded-xl text-sm text-amber-800 flex items-start gap-3 shadow-sm">
					...
					<p className="leading-relaxed">
						<strong>Notice:</strong> Standard fields like Base Salary and
						Government Deductions are locked by default. If you need to make
						manual adjustments for this payroll period, simply click the{" "}
						<strong>pencil icon</strong> next to the field to unlock it.
					</p>
				</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div className="bg-slate-50/50 border border-slate-200 rounded-xl p-6 shadow-sm h-fit">
							<div className="flex items-center gap-2 border-b border-slate-200 pb-3 mb-4">
								...
								<h3 className="font-semibold text-lg text-slate-800 m-0">
									Earnings
								</h3>
							</div>
							...
							<div className="grid grid-cols-2 gap-4">
								...
								{type === "part_time_teaching" && (
									<>
										<div className="p-3 bg-blue-50 text-blue-800 text-xs rounded-md border border-blue-200 col-span-2">
											<strong>Note:</strong> You must enter the{" "}
											<em>Number of Classes (Cut-off)</em> to calculate the
											gross earnings for this period.
										</div>
										<CurrencyInput
											id="dailySalary"
										...
										/>
										<CurrencyInput
											id="subjectHoursPerDay"
											label="Subject Hours Per Day"
											value={formData.subjectHoursPerDay}
											onChange={handleChange}
											disabled
										/>
										<CurrencyInput
											id="numberOfClasses"
											label="Classes (Cut-off)"
								...
								{type === "non_teaching" && (
									<CurrencyInput
										...
										value={formData.transpoAllowance}
										onChange={handleChange}
									/>
								)}
								...
								{(type === "teaching" || type === "non_teaching") && (
									<CurrencyInput
										id="subjectOverload"
										label="Subject Overload"
										value={formData.subjectOverload}
										onChange={handleChange}
									/>
								)}
						...
						{type !== "part_time_teaching" && (
							<div className="bg-slate-50/50 border border-slate-200 rounded-xl p-6 shadow-sm space-y-5 h-fit">
								...
								<div className="grid grid-cols-2 gap-4">
									<CurrencyInput
										id="philhealthEe"
										label="PhilHealth (EE Share)"
										value={formData.philhealthEe}
										onChange={handleChange}
										disabled
									/>
									<CurrencyInput
										id="philhealthEr"
										label="PhilHealth (ER Share)"
										value={formData.philhealthEr}
										onChange={handleChange}
										disabled
									/>
								</div>
								<div className="grid grid-cols-2 gap-4">
									<CurrencyInput
										id="pagIbigEe"
										label="Pag-IBIG (EE Share)"
										value={formData.pagIbigEe}
										onChange={handleChange}
										disabled
									/>
									<CurrencyInput
										id="pagIbigEr"
										label="Pag-IBIG (ER Share)"
										value={formData.pagIbigEr}
										onChange={handleChange}
										disabled
									/>
								</div>
