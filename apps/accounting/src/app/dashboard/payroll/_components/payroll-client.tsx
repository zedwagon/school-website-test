...
} from "@school/api/accounting/payroll/action";
import {
	Badge,
	Button,
	Dialog,
...
export function PayrollClient({
	...
	const initialYear = initialPeriodId
		? getSafeYear(
				periods.find((p) => p.id === initialPeriodId)?.startDate ||
					new Date().getFullYear().toString(),
			)
		: periods.length > 0
			? getSafeYear(periods[0].startDate)
	...
	}, [initialPeriodId, periods, getSafeYear]);
	...
	const availableYears = Array.from(
		new Set(periods.map((p) => getSafeYear(p.startDate))),
	).sort((a, b) => b - a);

	// Filter periods by selected year
	const filteredPeriods = periods.filter(
		(p) => getSafeYear(p.startDate) === selectedYear,
	);
	...
	filteredPeriods.forEach((p) => {
		...
		if (
			...
			groupedPeriods[groupedPeriods.length - 1].label !== month
		) {
		...
		<div className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 gap-4">
			<div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
					...
					<SelectContent>
						{groupedPeriods.map((group) => (
							<SelectGroup key={group.label}>
								<SelectLabel className="bg-slate-100/50">
									{group.label}
								</SelectLabel>
								{group.periods.map((p) => {
									...
									const end = new Date(p.endDate).toLocaleDateString("en-US", {
										month: "short",
										day: "numeric",
									});
									return (
				...
				{selectedPeriodObj && (
					<Badge
						...
						className={`ml-2 px-3 py-1 ${isFinalized ? "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-50" : "bg-amber-100 text-amber-800 hover:bg-amber-200 border-none"}`}
					>
			...
			<div className="flex flex-wrap items-center gap-2 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-slate-100">
				...
				{selectedPeriodObj && selectedPeriodObj.status === "DRAFT" && (
					<Button
						variant="outline"
						className="border-emerald-600 text-emerald-700 hover:bg-emerald-50 bg-white shadow-sm"
				...
				{selectedPeriodObj && selectedPeriodObj.status === "FINALIZED" && (
					<Button
						variant="outline"
						className="border-amber-600 text-amber-700 hover:bg-amber-50 bg-white shadow-sm"
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Are you sure you want to generate payroll for this period? This
							will calculate salaries for all active contracts and might
							overwrite existing un-finalized data.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<Button
