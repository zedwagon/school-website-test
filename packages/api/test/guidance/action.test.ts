...
describe("Guidance Department Actions", () => {
	...
	beforeAll(async () => {
		...
		const { users, students, schoolYears, enrollmentForms } = await import(
			"@school/db"
		);
	...
	afterAll(async () => {
		if (testFormId) {
			const { db } = await import("@school/db");
			const { enrollmentForms, students, schoolYears, users } = await import(
				"@school/db"
			);
			const { eq } = await import("drizzle-orm");

			await db
				.delete(enrollmentForms)
				.where(eq(enrollmentForms.id, testFormId));
