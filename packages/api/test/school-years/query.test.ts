...
describe("School Years CRUD and Actions Integration", () => {
	it("should create, update, archive, and restore a school year", async () => {
		...
		const [newSy] = await db
			.select()
			.from(schoolYears)
			.where(eq(schoolYears.name, syName));
		...
		const [updatedSy] = await db
			.select()
			.from(schoolYears)
			.where(eq(schoolYears.id, newSy.id));
		expect(updatedSy.name).toBe(`${syName} Updated`);

		// 3. Archive
		await archiveSchoolYearQuery(newSy.id);
		const [archivedSy] = await db
			.select()
			.from(schoolYears)
			.where(eq(schoolYears.id, newSy.id));
		...
		const [targetSy] = await db
			.select()
			.from(schoolYears)
			.where(eq(schoolYears.name, targetSyName));
