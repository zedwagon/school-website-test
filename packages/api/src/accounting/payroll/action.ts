...
export async function createPayrollPeriod(data: {
	...
	const existing = await db
		.select()
		.from(payrollPeriods)
		.where(eq(payrollPeriods.name, data.name));

	if (existing.length > 0) {
		throw new Error(
			`A payroll period for ${data.startDate} to ${data.endDate} already exists.`,
		);
	}
...
export async function generatePayrollsForPeriod(periodId: number) {
	...
	if (contractsQuery.length === 0) {
		throw new Error(
			"No employee contracts found. Please set up contracts first.",
		);
	}
...
export async function updatePayroll(
	id: number,
	data: Partial<typeof payrolls.$inferInsert>,
) {
	// Recalculate totals if any specific values change
	const current = await db.select().from(payrolls).where(eq(payrolls.id, id));
	...
	if (type === "administrators") {
		...
		totalEarn =
			safeParseFloat(merged.halfSalary) +
			safeParseFloat(merged.subjectOverload) +
			safeParseFloat(merged.positionPay) +
			safeParseFloat(merged.advisoryPay);
	} else if (type === "non_teaching") {
		totalEarn =
			safeParseFloat(merged.halfSalary) +
			safeParseFloat(merged.additionalPay) +
			safeParseFloat(merged.subjectOverload) +
			safeParseFloat(merged.transpoAllowance) +
			safeParseFloat(merged.holidayPay) +
			safeParseFloat(merged.moderatorPay);
	} else if (type === "part_time_teaching") {
		totalEarn =
			parseInt(merged.numberOfClasses || "0", 10) *
			safeParseFloat(merged.dailySalary);
	}

	const totalDed =
		type === "part_time_teaching"
			? 0
			: safeParseFloat(merged.sssEe) +
				safeParseFloat(merged.philhealthEe) +
				safeParseFloat(merged.pagIbigEe) +
				safeParseFloat(merged.loanDeduction);
	const net = totalEarn - totalDed;
